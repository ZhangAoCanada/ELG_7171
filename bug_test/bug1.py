...
class BugOne(object):
    ...
    #                               Bug 1 Alogrithm                              #
    ##############################################################################
    def EncounterObstacle(self, range_min):
        """
        Find out if a obstacle is encountered or not.
        """
        if range_min <= OBSTACLE_THRESHOLD:
            obstacle_ahead = True
        else:
    ...
    def MoveToTarget(self, robot_pose):
        """
    ...
    def BugOne(self, robot_pose, ranges, angles, range_min, angle_min):
        ...
        if self.GoalReached(robot_pose):
            ...
            if range_min is None and not self.EncounterObstacle(range_min):
                v, w = self.MoveToTarget(robot_pose)
            else:
